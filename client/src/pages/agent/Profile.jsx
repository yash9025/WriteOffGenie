...
import { doc, updateDoc, collection, addDoc, deleteDoc, query, orderBy, onSnapshot, writeBatch } from "firebase/firestore";
...
const Modal = ({ title, onClose, children }) => (
  <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
    <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl relative scale-100 transition-all">
...
const Input = ({ label, type = "text", value, onChange, placeholder, disabled, maxLength }) => (
  <div className="flex flex-col gap-1.5">
    ...
    <input 
      ...
      disabled={disabled}
      maxLength={maxLength}
      className="w-full border border-[#E3E6EA] rounded-lg px-4 py-2.5 text-sm text-[#111] placeholder:text-gray-400 focus:outline-none focus:border-[#011C39] focus:ring-1 focus:ring-[#011C39] disabled:bg-gray-50 disabled:text-gray-500 transition-all"
    />
  </div>
);

const Button = ({ children, variant = "primary", onClick, loading, className }) => {
  ...
  const styles = variant === "primary" 
    ...
    : "border border-[#E3E6EA] text-[#111] hover:bg-gray-50";
  
  return (
    <button onClick={onClick} disabled={loading} className={`${baseStyle} ${styles} disabled:opacity-70 disabled:cursor-not-allowed ${className}`}>
...
const Profile = () => {
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-full animate-in fade-in duration-500 pb-10">
      ...
      {/* Header */}
      <div>
        <h1 className="text-[#111111] text-2xl font-bold tracking-tight">My Profile</h1>
        <p className="text-[#9499A1] text-sm mt-1">View and manage your personal information</p>
      </div>
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="relative w-24 h-24 shrink-0">
            <div className="w-24 h-24 rounded-full bg-[#011C39] flex items-center justify-center text-white text-3xl font-bold border-4 border-white shadow-sm">
              {partnerData?.displayName?.[0]?.toUpperCase() || "A"}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 flex-1">
            <div>
              ...
              <p className="text-base font-medium text-[#111]">{partnerData?.displayName || '-'}</p>
            </div>
            <div>
              <p className="text-xs font-medium text-[#9499A1] mb-1 uppercase tracking-wide">Email address</p>
              <p className="text-base font-medium text-[#111]">{user?.email || '-'}</p>
            </div>
            <div>
              <p className="text-xs font-medium text-[#9499A1] mb-1 uppercase tracking-wide">Phone number</p>
              <p className="text-base font-medium text-[#111]">{partnerData?.phoneNumber || '-'}</p>
            </div>
            <div>
              <p className="text-xs font-medium text-[#9499A1] mb-1 uppercase tracking-wide">Business Name</p>
              <p className="text-base font-medium text-[#111]">{partnerData?.businessName || '-'}</p>
            </div>
            <div>
              <p className="text-xs font-medium text-[#9499A1] mb-1 uppercase tracking-wide">Referral Code</p>
              <p className="text-base font-medium text-[#111] font-mono">{partnerData?.referralCode || '-'}</p>
            </div>
            <div className="lg:col-span-3">
              <p className="text-xs font-medium text-[#9499A1] mb-1 uppercase tracking-wide">Address</p>
              <p className="text-base font-medium text-[#111]">{partnerData?.address || '-'}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {showEditProfile && (
        <Modal title="Update Profile" onClose={() => setShowEditProfile(false)}>
           <div className="flex flex-col gap-4">
              ...
              />
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-medium text-[#111]">Address</label>
                <textarea 
                  value={profileForm.address} 
                  onChange={e => setProfileForm({...profileForm, address: e.target.value})}
                  rows={3}
                  className="w-full border border-[#E3E6EA] rounded-lg px-4 py-2.5 text-sm text-[#111] placeholder:text-gray-400 focus:outline-none focus:border-[#011C39] focus:ring-1 focus:ring-[#011C39] transition-all resize-none"
                />
              </div>
