...
function ClientRegister() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 2. Show Success Message (STOP HERE - No Redirect)
      setSuccess(true);

      // Optional: Clear form but keep referral code
      setForm((prev) => ({
        ...prev,
        password: "",
        name: "",
  ...
  return (
    // ✅ Applied exact same gradient background as CA Register
      ...
      <div className="max-w-md w-full bg-white rounded-2xl shadow-2xl overflow-hidden border border-white/20">
        <div className="px-8 pt-8 pb-4 text-center">
          ...
          <p className="text-slate-500 text-sm mt-2">
            Get your taxes sorted with WriteOffGenie.
          </p>
        </div>

        <div className="px-8 pb-6">
          {/* ✅ SUCCESS MESSAGE (Only shows if success is true) */}
          {success && (
          ...
          {!success && (
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {/* Updated Button to match CA Register Style */}
              <button
                disabled={loading}
